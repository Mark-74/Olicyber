...
class Searcher {
    ...
    function __construct($searchTerm, $history) {
        ...
        $this->history = $history; //questa viene passata come session
        $this->sendAnalytics();
    ...
    function sendAnalytics() { // io non vedo la risposta, quindi non credo sia importante
        $client = new GuzzleHttp\Client();
        $analytics = array(
            'search' => $this->searchTerm,         // anche questo
            'history' => $this->history->searches // queste le controllo io in realtà
        );
    ...
    function fetchResults() {
        ...
        $res = $client->request('GET', $_ENV['GOLANG_URL'] . '?search=' . $src, array( // gabibbo;debug=true
            'headers' => array(
